...
## Senior Review Follow-Ups (Deferred)

These were flagged during the senior review pass but not changed because the fix is not a small surgical edit and would benefit from explicit discussion.

### Use-Case Layer Bypassed In Several ViewModels

`SettingsViewModel`, `RecurringViewModel`, `AddRecurringRuleViewModel`, and `CategoriesViewModel` call repositories directly for write paths instead of going through dedicated use cases. `Architecture.md` recommends a `CreateRecurringExpenseUseCase` (and similar) that would centralise validation and make those flows independently testable.

Resolution: introduce `SaveBudgetUseCase`, `SaveRecurringRuleUseCase`, `ToggleRecurringRulePauseUseCase`, `DeleteRecurringRuleUseCase`, and a `SaveCategoryUseCase`. Move the model `init {}` validations into use case results so error messages can be surfaced without try/catch.

### Custom Category Insert Is Not Transactional

`CategoryRepositoryImpl.addCustomCategory` does `MAX(id) + 1` then `insert` in two separate calls. Two simultaneous custom-category creations can collide on the primary key.

Resolution: change `CategoryEntity.id` to `autoGenerate = true` (requires a Room migration since the entity has used non-autogenerated IDs since version 1) and let SQLite assign IDs. Seeded defaults already use fixed IDs and would need to be reserved (e.g. start the autoincrement counter at a higher value).
...
The Currency row in Settings uses the same `SettingRow` style as tappable rows but has no click handler. Either make it inert visually (no chevron, dim affordance) or wire it to a sheet showing supported currencies and an explicit "NPR-only in MVP" note.
